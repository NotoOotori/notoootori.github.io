...
- 除去几乎处处收敛或能推出依测度收敛之外, 没有别的推出关系.
- 令$$\displaystyle x_n=\frac{1}{n}\chi_{[0, n]}$$, 则序列$$\{x_n\}$$几乎处处收敛并依测度收敛到函数$$x=0$$, 但不依$$L^1$$范数收敛到$$x$$.
- 记$$\displaystyle I_{nm}=[\frac{1}{m}\sum_{j=1}^n\frac{1}{j}, \frac{1}{m}\sum_{j=1}^{n+1}\frac{1}{j}]$$, 将$$I_{nm}$$排成一列, 记为$$I_k$$, 定义$$x_k=\chi_{I_k}$$, $$x=0$$, 则序列$$\{x_k\}$$依测度收敛并依$$L^1$$范数收敛到$$x$$, 但是不几乎处处收敛到$$x$$.([Wikipedia](https://en.wikipedia.org/wiki/Convergence_in_measure)中给出了一种不需要重排的构造, 思路大同小异)
