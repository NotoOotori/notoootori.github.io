...
> **证明($$\mathrm{AC}\Rightarrow\mathrm{DC}$$)** 设$$X$$是非空集合, $$R$$是$$X$$上的整二元关系. 设$$a\in X$$, 定义$$R(a)=\left\{b\middle\vert\ aRb\right\}$$, 由$$R$$的整性得每个$$R(a)$$均非空, 故我们得到了集族$$\{R(a)\}_{a\in X}$$. 根据选择公理, 存在映射$$f: X\to \bigcup R(a)\subseteq X$$使得$$f(a)\in R(a)$$成立, 即$$aRf(a)$$成立. 再利用分离公理, 得到集合$$\left\{(n, x)\in\mathbb{N}\times X\middle\vert\ x=f^{n}(a)\right\}$$即为所求数列$$\{a, f(a), f^2(a), \dotsc, f^n(a), \dotsc\}$$, 其中$$f^n$$表示映射的$$n$$次复合. 证毕.
